 ...
 */
function checkReactCompatibility(version: string | null): CompatibilityResult {
  ...
  if (compareVersions(parsed, react19) >= 0) {
    ...
    result.warnings.push(
      `React ${version} is in legacy support mode. Consider upgrading to React >=19.0.0 for full feature support.`
    );
  } else {
    result.errors.push(
      `React ${version} is not supported. Please upgrade to React >=18.0.0 (preferably >=19.0.0)`
    );
  }
 ...
 */
function checkReactNativeCompatibility(version: string | null): CompatibilityResult {
  const result: CompatibilityResult = {
    isSupported: false,
    isLegacy: false,
    warnings: [],
  ...
  if (compareVersions(parsed, rn79) >= 0) {
    ...
    result.isLegacy = true;
    result.warnings.push(
      `React Native ${version} is in legacy support mode. Consider upgrading to React Native >=0.79.0 for full feature support.`
    );
  } else {
    result.errors.push(
      `React Native ${version} is not supported. Please upgrade to React Native >=0.70.0 (preferably >=0.79.0)`
    );
  }
 ...
 */
function checkSkiaCompatibility(
  ...
  if (reactNativeVersion) {
    ...
    if (rnParsed) {
      ...
      if (isRN78OrBelow && !isSkia1124OrBelow) {
        result.errors.push(
          `React Native Skia ${skiaVersion} is incompatible with React Native ${reactNativeVersion}. ` +
  ...
  if (compareVersions(skiaParsed, skia2) >= 0) {
    ...
    result.isLegacy = true;
    result.warnings.push(
      `React Native Skia ${skiaVersion} is in legacy support mode. Consider upgrading to >=2.0.0 for full feature support.`
    );
  } else {
    result.errors.push(
      `React Native Skia ${skiaVersion} is not supported. Please upgrade to @shopify/react-native-skia >=1.0.0`
    );
  }
 ...
 */
export function checkCompatibility(): FullCompatibilityResult {
 ...
 */
export function logCompatibilityStatus(silent = false): boolean {
 ...
 */
export function getRecommendedSkiaVersion(reactNativeVersion?: string): string {
  if (!reactNativeVersion) {
    return '>=2.0.0';
  }
 ...
 */
export function supportsVideoFeatures(): boolean {
