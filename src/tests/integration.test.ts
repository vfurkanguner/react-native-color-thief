...
import { formatRGB } from '../color-converters';
import { createMockPixelData, createMockSkiaImage, createMockSkiaCanvas, createMockSkiaSurface, TEST_COLORS, TEST_URLS } from './setup';
...
// Mock quantize
jest.mock('quantize', () => {
  return jest.fn().mockImplementation((pixels: number[][]) => {
...
describe('Integration Tests', () => {
  let colorThief: ReactNativeColorThief;
  ...
  beforeEach(() => {
    ...
    for (let i = 0; i < 10000; i++) {
      ...
      switch (colorIndex) {
        case 0:
          pixelData[offset] = 255; pixelData[offset + 1] = 0; pixelData[offset + 2] = 0; pixelData[offset + 3] = 255; // Red
          break;
        case 1:
          pixelData[offset] = 0; pixelData[offset + 1] = 255; pixelData[offset + 2] = 0; pixelData[offset + 3] = 255; // Green
          break;
        case 2:
          pixelData[offset] = 0; pixelData[offset + 1] = 0; pixelData[offset + 2] = 255; pixelData[offset + 3] = 255; // Blue
          break;
        case 3:
          pixelData[offset] = 255; pixelData[offset + 1] = 87; pixelData[offset + 2] = 51; pixelData[offset + 3] = 255; // Orange
          break;
        case 4:
          pixelData[offset] = 51; pixelData[offset + 1] = 255; pixelData[offset + 2] = 255; pixelData[offset + 3] = 255; // Cyan
          break;
      }
    }

    const mockCanvas = createMockSkiaCanvas();
    const mockImage = createMockSkiaImage(pixelData, 100, 100);
    const mockSurface = createMockSkiaSurface(mockCanvas, mockImage);
  ...
  describe('End-to-End Color Extraction', () => {
    ...
    it('should handle different image formats consistently', async () => {
      const formats = [TEST_URLS.JPG, TEST_URLS.PNG, TEST_URLS.SVG];
      
      const results = await Promise.all(
        formats.map(url => colorThief.getProminentColors(url))
      );
  ...
  describe('Color Format Integration', () => {
    it('should maintain color accuracy across format conversions', async () => {
      const colors = await colorThief.getProminentColors(TEST_URLS.JPG);
      ...
      colors.forEach(color => {
        ...
        expect(formats.hex).toBe(formatRGB(rgb, 'hex'));
        expect(formats.rgb).toBe(formatRGB(rgb, 'rgbString'));
        expect(formats.hsl).toBe(formatRGB(rgb, 'hslString'));
      });
    });

    it('should handle edge case colors correctly', async () => {
      // Mock edge case colors
      const edgeCasePixels = createMockPixelData([
        [0, 0, 0],       // Pure black
      ...
      const mockImage = createMockSkiaImage(edgeCasePixels);
      const mockCanvas = createMockSkiaCanvas();
      const mockSurface = createMockSkiaSurface(mockCanvas, mockImage);
  ...
  describe('Performance and Memory Management', () => {
    it('should handle multiple sequential calls without memory leaks', async () => {
      const urls = [TEST_URLS.JPG, TEST_URLS.PNG, TEST_URLS.SVG];
    ...
    it('should handle concurrent calls correctly', async () => {
      const promises = Array.from({ length: 5 }, (_, i) => 
        colorThief.getProminentColors(`${TEST_URLS.JPG}?v=${i}`)
      );
  ...
  describe('Error Recovery and Resilience', () => {
    it('should handle partial failures gracefully', async () => {
      ...
      Skia.Surface.MakeOffscreen.mockImplementation(() => {
        ...
        const mockCanvas = createMockSkiaCanvas();
        const mockImage = createMockSkiaImage(createMockPixelData([TEST_COLORS.RED]));
        return createMockSkiaSurface(mockCanvas, mockImage);
      });

      // First call should succeed
      try {
        const colors1 = await colorThief.getProminentColors(TEST_URLS.JPG);
      ...
      await expect(colorThief.getProminentColors(TEST_URLS.PNG))
        .rejects.toThrow('Failed to create offscreen surface');

      // Third call should succeed again
      try {
        const colors3 = await colorThief.getProminentColors(TEST_URLS.SVG);
    ...
    it('should maintain state consistency after errors', async () => {
      ...
      await expect(colorThief.getProminentColors(TEST_URLS.JPG))
        .rejects.toThrow();
  ...
  describe('Real-world Scenarios', () => {
    ...
    it('should provide useful color statistics for UI theming', async () => {
      const stats = await colorThief.getColorStatistics(TEST_URLS.JPG);
