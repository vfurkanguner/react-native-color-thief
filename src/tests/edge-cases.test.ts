...
import { createMockPixelData, createMockSkiaImage, createMockSkiaCanvas, createMockSkiaSurface, TEST_COLORS } from './setup';
...
jest.mock('quantize', () => {
  return jest.fn().mockImplementation((pixels: number[][]) => {
    ...
    const uniqueColors = pixels.filter((color, index, arr) => 
      arr.findIndex(c => c[0] === color[0] && c[1] === color[1] && c[2] === color[2]) === index
    );
...
describe('Edge Cases and Error Scenarios', () => {
  ...
  beforeEach(() => {
    ...
    for (let i = 0; i < 10000; i++) {
      ...
      switch (colorIndex) {
        case 0:
          defaultPixelData[offset] = 255; defaultPixelData[offset + 1] = 87; defaultPixelData[offset + 2] = 51; defaultPixelData[offset + 3] = 255;
          break;
        case 1:
          defaultPixelData[offset] = 51; defaultPixelData[offset + 1] = 255; defaultPixelData[offset + 2] = 87; defaultPixelData[offset + 3] = 255;
          break;
        case 2:
          defaultPixelData[offset] = 87; defaultPixelData[offset + 1] = 51; defaultPixelData[offset + 2] = 255; defaultPixelData[offset + 3] = 255;
          break;
      }
    }

    const mockCanvas = createMockSkiaCanvas();
    const mockImage = createMockSkiaImage(defaultPixelData, 100, 100);
  ...
  describe('Empty and Invalid Data', () => {
    it('should handle empty pixel data gracefully', async () => {
      ...
      const mockImage = createMockSkiaImage(emptyPixels, 0, 0);
      const mockCanvas = createMockSkiaCanvas();
      const mockSurface = createMockSkiaSurface(mockCanvas, mockImage);
    ...
    it('should handle single pixel images', async () => {
      const singlePixel = createMockPixelData([TEST_COLORS.RED]);
      const mockImage = createMockSkiaImage(singlePixel, 1, 1);
      const mockCanvas = createMockSkiaCanvas();
      const mockSurface = createMockSkiaSurface(mockCanvas, mockImage);
      ...
      expect(colors[0].rgb).toEqual(TEST_COLORS.RED);
    });
    ...
    it('should handle images with all transparent pixels', async () => {
      ...
      const mockImage = createMockSkiaImage(transparentPixels, 3, 1);
      const mockCanvas = createMockSkiaCanvas();
      const mockSurface = createMockSkiaSurface(mockCanvas, mockImage);
    ...
    it('should handle images with only white pixels when excludeWhite is true', async () => {
      const whitePixels = createMockPixelData([
        [255, 255, 255],
        [254, 254, 254],
        [253, 253, 253],
      ]);
      
      const mockImage = createMockSkiaImage(whitePixels);
      const mockCanvas = createMockSkiaCanvas();
      const mockSurface = createMockSkiaSurface(mockCanvas, mockImage);
  ...
  describe('Extreme Configuration Values', () => {
    ...
    it('should handle very large quality values', async () => {
      const testPixels = createMockPixelData([TEST_COLORS.RED]);
      const mockImage = createMockSkiaImage(testPixels, 1, 1);
      const mockCanvas = createMockSkiaCanvas();
      const mockSurface = createMockSkiaSurface(mockCanvas, mockImage);
  ...
  describe('Quantization Edge Cases', () => {
    it('should handle quantization returning null', async () => {
      ...
      const testPixels = createMockPixelData([TEST_COLORS.RED]);
      const mockImage = createMockSkiaImage(testPixels);
      const mockCanvas = createMockSkiaCanvas();
      const mockSurface = createMockSkiaSurface(mockCanvas, mockImage);
